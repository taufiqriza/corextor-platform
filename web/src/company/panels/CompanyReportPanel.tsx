...
function statusMeta(status: string, T: Theme) {
    switch (status) {
        case 'present': return { label: 'Hadir', bg: `${T.success}14`, color: T.success };
        case 'corrected': return { label: 'Dikoreksi', bg: `${T.info}14`, color: T.info };
        default: return { label: status, bg: `${T.textMuted}10`, color: T.textMuted };
    }
}

export function CompanyReportPanel({ T, isDesktop }: Props) {
    const [records, setRecords] = useState<any[]>([]);
    const [stats, setStats] = useState({ total: 0, present: 0, corrected: 0, complete: 0, ongoing: 0 });
    ...
    const filtered = useMemo(() => {
        let list = records;
        if (search.trim()) { const q = search.toLowerCase(); list = list.filter(r => r.employee_name?.toLowerCase().includes(q) || r.branch_name?.toLowerCase().includes(q) || r.date?.includes(q)); }
    ...
    const handleCorrection = async () => {
        ...
        try {
            await attendanceApi.correctAttendance(correcting.id, { time_in: corrTimeIn || undefined, time_out: corrTimeOut || undefined, note: corrNote || undefined });
    ...
    const s = {
        ...
        searchWrap: { flex: 1, minWidth: isDesktop ? 200 : '100%', display: 'flex', alignItems: 'center', gap: 8, height: 40, borderRadius: 11, border: `1px solid ${T.border}`, background: T.bgAlt, padding: '0 10px' } as React.CSSProperties,
        ...
        <div style={{ display: 'grid', gap: 14 }}>
            ...
            <section style={s.section}>
                <div style={{ display: 'flex', alignItems: isDesktop ? 'center' : 'flex-start', justifyContent: 'space-between', gap: 10, flexDirection: isDesktop ? 'row' : 'column' }}>
                    ...
                    <button onClick={load} style={{ height: 40, padding: '0 14px', borderRadius: 11, background: T.bgAlt, color: T.textSub, fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, border: `1px solid ${T.border}` }}>
                ...
                <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    <div style={s.searchWrap}>
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari..." style={{ flex: 1, color: T.text, fontSize: 13 }} />
                ...
                {loading ? (
                    ...
                    <div style={{ padding: 40, textAlign: 'center' }}><p style={{ fontSize: 13, color: T.textMuted }}>{search ? 'Tidak ditemukan.' : 'Tidak ada data.'}</p></div>
                ) : isDesktop ? (
                                    ...
                                    <tr key={r.id} style={{ background: T.card, transition: 'background .15s' }}
                                        ...
                                        <td style={s.td}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                ...
                                                <span style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{r.employee_name}</span>
                                            </div>
                                        </td>
                                        <td style={{ ...s.td, fontSize: 12, color: T.textSub }}>{r.branch_name ?? '-'}</td>
                                        <td style={{ ...s.td, fontSize: 12, color: T.textSub }}>{formatDate(r.date)}</td>
                                        <td style={s.td}><span style={{ fontSize: 13, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: r.time_in ? T.success : T.textMuted }}>{r.time_in ?? '—'}</span></td>
                                        <td style={s.td}><span style={{ fontSize: 13, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: r.time_out ? T.info : T.textMuted }}>{r.time_out ?? '—'}</span></td>
                                        <td style={s.td}>
                                            <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 9px', borderRadius: 999, background: sm.bg, color: sm.color }}>{sm.label}</span>
                                            {!r.time_out && r.time_in && <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 9px', borderRadius: 999, background: `${T.gold}14`, color: T.gold, marginLeft: 4 }}>Ongoing</span>}
                                        </td>
                                        <td style={s.td}>
                                            <button onClick={() => { setCorrecting(r); setCorrTimeIn(r.time_in ?? ''); setCorrTimeOut(r.time_out ?? ''); setCorrNote(''); setCorrMsg(''); }} style={{
                            ...
                            return (
                                <div key={r.id} style={s.mCard}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        ...
                                        <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 999, background: sm.bg, color: sm.color }}>{sm.label}</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: 16, marginTop: 10, alignItems: 'center' }}>
                                        <div>
                                            <div style={{ fontSize: 9, color: T.textMuted, fontWeight: 700, textTransform: 'uppercase' }}>In</div>
                                            <div style={{ fontSize: 14, fontWeight: 800, color: T.success, fontFamily: "'JetBrains Mono', monospace" }}>{r.time_in ?? '—'}</div>
                                        </div>
                                        <div>
                ...
                <div style={s.modal} onClick={() => setCorrecting(null)}>
                    <div style={s.modalBox} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                            <div><h3 style={{ fontSize: 16, fontWeight: 900, color: T.text, fontFamily: "'Sora', sans-serif" }}>Koreksi Absensi</h3><p style={{ fontSize: 11, color: T.textMuted, marginTop: 3 }}>{correcting.employee_name} — {formatDate(correcting.date)}</p></div>
                            <button onClick={() => setCorrecting(null)} style={{ color: T.textMuted }}><X size={18} /></button>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                            <div><label style={{ fontSize: 10, fontWeight: 700, color: T.textMuted, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Check-in</label><input type="time" step="1" value={corrTimeIn} onChange={e => setCorrTimeIn(e.target.value)} style={s.input} /></div>
                            <div><label style={{ fontSize: 10, fontWeight: 700, color: T.textMuted, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Check-out</label><input type="time" step="1" value={corrTimeOut} onChange={e => setCorrTimeOut(e.target.value)} style={s.input} /></div>
                        </div>
                        <div style={{ marginBottom: 16 }}>
                            ...
                            <textarea value={corrNote} onChange={e => setCorrNote(e.target.value)} placeholder="Alasan koreksi..." rows={3} style={{ ...s.input, height: 'auto', padding: '10px 12px', resize: 'none' }} />
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                            ...
                            <button onClick={handleCorrection} disabled={corrSaving} style={{ height: 42, padding: '0 22px', borderRadius: 11, background: T.primary, color: '#fff', fontSize: 12, fontWeight: 700, opacity: corrSaving ? .6 : 1 }}>{corrSaving ? 'Menyimpan...' : '✓ Simpan'}</button>
                        </div>
                    </div>
                </div>
            )}
